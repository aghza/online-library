	...
	<div class="col-lg-12">
        ...
        <div class="small-container">
            ...
            if(isset($_POST['submit']))
            {
                ...
                ?>
                <h2 class='all-files-title'>
                <?php
                echo $row['categoryname'];
                ?></h2>
                <?php     
                $q=mysqli_query($db,"SELECT books.bookid,books.bookpic,books.bookname,category.categoryname,folderes.name ,status from  `files` join `category` on category.categoryid=books.categoryid join `folders` on authors.authorid=books.authorid where bookname like '%$_POST[search]%' AND books.categoryid = $_POST[category];");
                
                if(mysqli_num_rows($q)==0)
                {
                    echo "Sorry! No Books found. Try searching again";u
                ...
                <?php 
                    ...
                    ?><h2 class="all-books-title">All Books</h2>
                <?php
                $q=mysqli_query($db,"SELECT books.bookid,books.bookpic,books.bookname,category.categoryname,folderes.name,,status from  `books` join `category` on category.categoryid=books.categoryid join `authors` on authors.authorid=books.authorid where bookname like '%$_POST[search]%'; ");
                if(mysqli_num_rows($q)==0)
                {
                ...
                ?><h2 class="all-books-title">All files</h2>
                <div class="row">
                <?php
                $res=mysqli_query($db,"SELECT files.id,filess.bookname,category.categoryname,folders.f_name,files.ISBN,ffrom  `books` join `category` on category.categoryid=books.categoryid join `authors` on authors.authorid=books.authorid;");
                while($row=mysqli_fetch_assoc($res)){
                    ...
                    <div class="card">
                        ...
                        ?>
                            ...
                            <a href="requested_book.php?req=<?php echo $row['id'];?>"><button type="submit" name="submit1" class="btn btn-default" >Request
                                </button>
                            </a>
                            </div>
                        </div>
...
<style>
.all-file .card:hover .overlay{
    opacity: 1;
}
.all-file .card:hover .sub-card{
    opacity: 1;
    bottom: 10%;
    /* position: relative; */
}
